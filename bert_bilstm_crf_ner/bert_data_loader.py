...
from bert_data_utils import create_dico, create_mapping, zero_digits
from bert_data_utils import iob2, iob_iobes

BERT_DIR = os.getenv("BERT_DIR", os.path.dirname(os.path.abspath(__file__)))
vocab_file = os.path.join(BERT_DIR, "pretrain/chinese_L-12_H-768_A-12/vocab.txt")
tokenizer = tokenization.FullTokenizer(vocab_file=vocab_file,
                                       do_lower_case=True)


def load_sentences(lower, zeros, data):
    """
    ...
    label = []
    num = 0
    for line in data:   # codecs.open(path, 'r', 'utf8'):
        num += 1
...
def read_dict_file(file_path):
    with open(file_path, 'r', encoding="utf-8") as fr:
        data = fr.read()
        word2id = json.loads(data, encoding="utf-8")
    return word2id